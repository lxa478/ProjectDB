...
class Node:
    ...
    def dump(self, indent=""):
        ...
        print
        indent,
        if flag == Rootflag: print
        "root",
        elif flag == Interiorflag: print
        "interior",
        elif flag == Leafflag: print
        "leaf",
        elif flag == LeafandRootflag: print
        "root and leaf",
        else: print
        "invalid flag???", flag,
        print
        self.position, "valid=", self.validkeys
        print
        indent, "keys", self.keys
        print
        indent, "seeks", self.indices
...
def testdbm():
    print
    "creating files test1, 2, 3 for dbm test"
    d1 = dbm("test1", "c")
    for x in range(10):
        key = "hello" * x
    ...
    len(d1), d1.keys()
    print
    "should be 0:", d1.has_key(""), d1.has_key("abd")
    print
    "copying"
    d2 = d1.copy("test2", "c")
    beforedel = len(d1)
    ...
    d2 = dbm("test2", "r")
    print
    "should be equal", beforedel - 1, len(d2)
    print
    "keys", d2.keys()
    print
    "testing copy-into"
    ...
    d3 = d2.copy("test3", "w")
    print
    "should be equal", beforedel, len(d3)
    print
    "keys", d3.keys()
...
def test():
    ...
    """
    print
    "creating file 'test' in current directory for test data."
    f = open("test", "w+b")
...
def retest():
    ...
    for x in "abcdefghi012345":
        try:
            ...
            x, "absent"
    print
    "entering torture chamber"
    s = time()
    for x in range(1000): l = B[hex(x)]
    print
    "1 thousand retrieves: ", time() - s
    return B
    print
    "keys, values between 4 and C (including C)"
    ...
    print
    print
    "keys, values between 4 (including 4) and C (excluding C)"
